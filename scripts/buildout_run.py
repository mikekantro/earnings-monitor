...
Pipeline per company:
  ...
  3. ai_highlight.get_buildout_highlight(...)     -> grounded snippet (or nothing),
...
try:
    from buildout_watch import get_buildout_highlight
except ImportError:
...
def main():
    ...
    ap.add_argument("--out", default="buildout_highlights.json")
    ...
    args = ap.parse_args()
    if args.model:
        os.environ["AI_HIGHLIGHT_MODEL"] = args.model  # shared env var
    ...
    for i, t in enumerate(tickers, 1):
        try:
            ...
            h = get_buildout_highlight(t, names.get(t, t), text, debug=args.debug)
            if h:
                h["event_date"] = edate
                hits.append(h)
                print(f"[{i}/{len(tickers)}] {t:6s} ✅ {h['channel']:24s} {h['headline'][:50]}")
            else:
                no_ai += 1  # counter name kept from template
                print(f"[{i}/{len(tickers)}] {t:6s} —  no build-out signal")
            done_f.write(t + "\n"); done_f.flush()
    ...
    print(f"\n{'='*55}")
    print(f"Build-out claimants found: {len(hits)}")
    from collections import Counter
    print("By channel:", dict(Counter(h["channel"] for h in hits)))
